...
public class BoletaController {
    ...
    @Operation(summary = "Emitir boleta con exportacion a PDF")
    @PostMapping("/emitir-con-pdf")
    public ResponseEntity<ApiResponse<String>> emitirBoletaConPDF(@Valid @RequestBody Boleta boleta) {
        log.info("POST /comprobantes/boletas/emitir-con-pdf - Emitiendo boleta con PDF");
        ComprobanteConPDF boletaConPDF = new ComprobanteConPDF(boleta);
        boletaConPDF.emitir();
        boletaService.guardarBoleta(boleta);
        return ResponseEntity.ok(ApiResponse.success("Boleta emitida con PDF", "Serie: " + boleta.getSerie()));
    }
}